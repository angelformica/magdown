...
//            $request->file('pdf_file')->move('uploads/pdf', $magazine->id.'.pdf');
	...
	public function updateMagazine(Request $request){
		 ...
		 * The magazine
		 */
		$magazine = Magazine::with('links')->findOrFail($request->get('id'));
		$magazine->update($data);
		$magazine->links()->delete();
		...
		foreach ($request->links as $link) {
			if($link){
				$links[] = new \App\Link(['url' => $link]);
			}
		}
		$magazine->links()->saveMany($links);
		...
		$error2 = ['The magazine has been updated successfully'];
		return $this->index($error2);
	}

    public function editMagazine($id){
	    $magazine = Magazine::findOrFail($id);
	    return view('magazine_edit', compact('magazine'));
    }
